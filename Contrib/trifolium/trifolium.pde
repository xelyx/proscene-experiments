/** trifolium : exercice sur les similitudes: scale, rotation,translation
 * Le trifolium zst extrudé par une suite de similitudes modifiées par les memes increments
 * Comme la croissance de la coquille de l'escargot
 * Exploitation des frames et des conversions de coordonnees ( ici inverseCoordinatesOf)
 * fait le 26/02/2016 par Jacques Maire
 **/
...
void setup() {
  ...
  frame1.scale(1.06);
  nf=60;//nombre de facettes sur les anneaux
  ptSim1=new Vec[nf];
  ptSim2=new Vec[nf];       
  ptRef=new Vec[nf];
  ...
  for (int i=0; i<nf; i++) {
     float th=TWO_PI*i/nf;
    ptRef[i]=new Vec(30*cos(th)*cos(3*th), 30*sin(th)*cos(3*th), 10*sq(cos(2*th)));
...
void enroulee(PGraphics pg) {
  ...
  Quat quatPas=Quat.multiply(new Quat(qua.axis(), qua.angle()*0.1), 
    new Quat(new Vec(2*cos(3*tempo), -4*sin(0.5*tempo), 2+sin(2*tempo)), 0.3));
  Vec vecPas=Vec.multiply(frame1.position().get(), 0.04);
