...
void setup() {
  ...
  frame1.scale(1.06);
  nf=400;//nombre de facettes sur les anneaux
  ptSim1=new Vec[nf];
  ptSim2=new Vec[nf];
  ptRef=new Vec[nf];
  for (int i=0; i<nf; i++) {
    ptRef[i]=new Vec(40*cos(PI*i/nf), 40*sin(PI*i/nf), 40.0*i/nf*cos(PI*i/nf));
...
//dessin 
  for (int u=0; u<54; u++) {
    utile1.setPosition(tr);utile1.setOrientation(qu);utile1.setMagnitude(scalor);
    tr=Vec.add(tr, vecPas);
    qu=Quat.multiply(quatPas, qu);
    scalor=scalor*scalor0;
     utile2.setPosition(tr);utile2.setOrientation(qu);utile2.setMagnitude(scalor);
     
    //les coordonnées des points sur les 2 cercles formant l'anneau courant
    for (int t=0; t<nf; t++)
    {
      ptSim1[t]=ptSim2[t].get();
      ptSim1[t]=utile1.inverseCoordinatesOf(ptRef[t]);
      ptSim2[t]=utile2.inverseCoordinatesOf(ptRef[t]);
    }
    //dessin de l'anneau courant
    pg.noStroke();
    pg.normal(1, 1, 1);
    pg.beginShape(TRIANGLE_STRIP);
    
    for (int i=0; i<nf; i++) { 
      if(i%40<20)fill(0);else fill(255);
